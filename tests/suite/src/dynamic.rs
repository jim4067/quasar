...
fn build_dynamic_account_data(name: &[u8], tags: &[Address]) -> Vec<u8> {
    // Pod-dynamic layout:
    // [disc][u8:name_len][name_bytes][u16:tags_count][tag_elements]
...
fn build_mixed_account_data(authority: Address, value: u64, label: &[u8]) -> Vec<u8> {
    // Layout: [disc(1)][authority(32)][value(8)][u8:label_len][label_bytes]
...
fn build_small_prefix_account_data(tag: &[u8], scores: &[u8]) -> Vec<u8> {
    // Layout: [disc(1)][u8:tag_len][tag_bytes][u16:scores_count][score_elements]
    let tag_len = tag.len();
...
fn build_tail_str_account_data(authority: Address, label: &[u8]) -> Vec<u8> {
    // Pod-dynamic layout: [disc(1)][authority(32)][u8:label_len][label_bytes]
    let label_len = label.len();
    let total = 1 + TAIL_FIXED_SIZE + 1 + label_len;
    let mut data = vec![0u8; total];
    data[0] = TAIL_STR_DISC;
    data[1..33].copy_from_slice(authority.as_ref());
    data[33] = label_len as u8;
    data[34..34 + label_len].copy_from_slice(label);
    data
}

fn build_tail_bytes_account_data(authority: Address, payload: &[u8]) -> Vec<u8> {
    // Pod-dynamic layout: [disc(1)][authority(32)][u16:data_count][data_bytes]
    let payload_len = payload.len();
    let total = 1 + TAIL_FIXED_SIZE + 2 + payload_len;
    let mut data = vec![0u8; total];
    data[0] = TAIL_BYTES_DISC;
    data[1..33].copy_from_slice(authority.as_ref());
    data[33..35].copy_from_slice(&(payload_len as u16).to_le_bytes());
    data[35..35 + payload_len].copy_from_slice(payload);
    data
}
...
#[test]
fn test_dynamic_account_invalid_utf8_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();

    // Invalid UTF-8 byte — accepted because UTF-8 re-validation was removed.
    // Owner check proves the owning program wrote the data, and from_utf8_unchecked
    // is sound under the SVM zeroing invariant.
...
fn test_dynamic_instruction_invalid_utf8_rejected() {
    ...
    let instruction: Instruction = DynamicInstructionCheckInstruction {
        authority,
        name: quasar_lang::client::DynBytes::<u8>::new(vec![0xFF]),
    }
    .into();
...
fn test_dynamic_account_name_exceeds_max_rejected() {
    ...
    // Corrupt name length prefix (at offset 1, u8) to exceed max of 8
    data[1] = 100;
...
fn test_dynamic_account_too_small_for_prefixes() {
    ...
    // Only disc byte — not enough for the u8 name prefix + u16 tags prefix
...
fn test_mixed_account_truncated_in_dynamic_section() {
    ...
    // Corrupt: set label prefix (u8) to claim 100 bytes but data only has 5
...
#[test]
fn test_mixed_account_invalid_utf8_label_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
    let authority = Address::new_unique();

    // Invalid UTF-8 in label — accepted because UTF-8 re-validation was removed.
    // Owner check proves the owning program wrote the data, and from_utf8_unchecked
    // is sound under the SVM zeroing invariant.
...
#[test]
fn test_small_prefix_invalid_utf8_tag_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();

    // Invalid UTF-8 in tag — accepted because UTF-8 re-validation was removed.
    // Owner check proves the owning program wrote the data, and from_utf8_unchecked
    // is sound under the SVM zeroing invariant.
...
fn test_dynamic_mutate_same_length_name() {
    ...
    // Verify tags were preserved (u16 prefix)
...
fn test_dynamic_mutate_preserves_trailing_vec() {
    ...
    assert_eq!(
        &result_data[tags_offset + 34..tags_offset + 66],
        tag2.as_ref()
    );
}
...
// ============================================================================
// STACK-CACHE TESTS: load_NAME() + save_dynamic() batched flush
// ============================================================================

fn build_stack_cache_instruction(
    account: Address,
    payer: Address,
    system_program: Address,
    new_name: &[u8],
) -> Instruction {
    // Instruction data: [disc(57)][u8:name_len][name_bytes]
    let mut data = vec![57];
    data.push(new_name.len() as u8);
    data.extend_from_slice(new_name);
    Instruction {
...
/// Stack-cache pattern: load → mutate name → save_dynamic (batched flush).
/// Tests growing the name (realloc) while preserving tags via single write.
#[test]
fn test_stack_cache_grow_name_preserves_tags() {
    let mollusk = setup();
    let (system_program, system_program_account) = keyed_account_for_system_program();
    let account = Address::new_unique();
    let payer = Address::new_unique();
    ...
    let tag = Address::new_unique();
    // Start with name="hi" (2 bytes), tags=[tag] (1 element)
    let account_bytes = build_dynamic_account_data(b"hi", &[tag]);
    let account_data = Account {
        lamports: 1_000_000,
        data: account_bytes,
    ...
    assert!(
        result.program_result.is_ok(),
        "stack-cache grow should succeed: {:?}",
        result.program_result
    );

    let result_data = &result.resulting_accounts[0].1.data;
    assert_eq!(result_data[0], DYNAMIC_ACCOUNT_DISC);
    // Name changed from "hi" to "world"
    let name_len = result_data[1] as usize;
    assert_eq!(name_len, 5);
    assert_eq!(&result_data[2..7], b"world");
...
/// Stack-cache: same-length content change (no realloc needed).
/// This tests the critical case where content changes but length doesn't —
/// the unconditional write ensures content changes are NOT silently lost.
#[test]
fn test_stack_cache_same_length_content_change() {
    let mollusk = setup();
    let (system_program, system_program_account) = keyed_account_for_system_program();
    let account = Address::new_unique();
    let payer = Address::new_unique();

    // Start with name="hello" (5 bytes), tags=[]
    let account_bytes = build_dynamic_account_data(b"hello", &[]);
    let account_data = Account {
        lamports: 1_000_000,
        data: account_bytes,
    ...
    assert!(
        result.program_result.is_ok(),
        "same-length stack-cache mutate should succeed: {:?}",
        result.program_result
    );
    ...
    let name_len = result_data[1] as usize;
    assert_eq!(name_len, 5);
    // Content MUST be "world", not "hello" — verifies unconditional write
    assert_eq!(&result_data[2..7], b"world");
}
...
fn test_adversarial_prefix_u8_max_name_len() {
    ...
    let mut data = vec![0u8; 1 + 1 + 2]; // disc + u8 name prefix + u16 tags prefix
...
fn test_adversarial_prefix_one_past_max() {
    ...
    let mut data = vec![0u8; 1 + 1 + 9 + 2]; // disc + u8 prefix + "aaaaaaaaa" + u16 tags prefix
    data[0] = DYNAMIC_ACCOUNT_DISC;
    data[1] = 9;
...
/// path
#[test]
fn test_adversarial_vec_count_u16_max() {
    let mollusk = setup();
    let account = Address::new_unique();

    // DynamicAccount: name="" (u8 prefix=0), tags count=u16::MAX
    let mut data = vec![0u8; 1 + 1 + 2]; // disc + name prefix(0) + tags prefix
    data[0] = DYNAMIC_ACCOUNT_DISC;
    data[1] = 0; // empty name
...
fn test_adversarial_vec_count_one_past_max() {
    ...
    // Build with 3 tags (max=2): name="" u8 prefix=0, tags u16 count=3
...
fn test_adversarial_vec_data_truncated_mid_element() {
    ...
    // name="" (u8 prefix=0) + tags u16 count=1 but only 16 bytes (Address is 32)
...
/// Accepted because UTF-8 re-validation was removed — owner check proves the
/// owning program wrote the data, and from_utf8_unchecked is sound under the
/// SVM zeroing invariant.
#[test]
fn test_adversarial_utf8_truncated_2byte_sequence_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
...
/// SVM zeroing invariant.
#[test]
fn test_adversarial_utf8_truncated_3byte_sequence_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
...
/// SVM zeroing invariant.
#[test]
fn test_adversarial_utf8_overlong_nul_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
...
/// Surrogate half (ED A0 80 = U+D800) — invalid in UTF-8, but accepted because
/// UTF-8 re-validation was removed. Owner check proves the owning program wrote
/// the data, and from_utf8_unchecked is sound under the SVM zeroing invariant.
#[test]
fn test_adversarial_utf8_surrogate_half_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
...
fn test_adversarial_small_prefix_u8_max_value() {
    ...
    // disc + tag prefix(255) + 0 bytes of data + u16 scores prefix
    // tag prefix claims 255 bytes but max=100, so this should be rejected
    let mut data = vec![SMALL_PREFIX_DISC, 255];
    // Add enough data so the tag prefix doesn't cause AccountDataTooSmall first
    data.extend_from_slice(&[b'a'; 255]); // 255 bytes of tag
...
#[test]
fn test_adversarial_small_prefix_vec_u16_overflow() {
    let mollusk = setup();
    let account = Address::new_unique();

    // disc + tag(u8 prefix=0, empty) + scores(u16 prefix=255, max=10)
    let mut data = vec![SMALL_PREFIX_DISC, 0];
    data.extend_from_slice(&255u16.to_le_bytes()); // 255 scores, max=10
    ...
    assert_eq!(
        result.program_result,
        ProgramResult::Failure(ProgramError::InvalidAccountData),
        "u16 vec count=255 (max=10) must be rejected"
    );
}
...
fn test_adversarial_missing_second_prefix() {
    ...
    // Total: 2 bytes. Minimum needed: disc(1) + u8(1) + u16(2) = 4
...
/// Partial tags prefix: only 1 of 2 bytes for u16 prefix
#[test]
fn test_adversarial_partial_u16_prefix() {
    let mollusk = setup();
    let account = Address::new_unique();

    // disc + u8 name prefix(0) + 1 byte of what should be a 2-byte u16 prefix
    let data = vec![DYNAMIC_ACCOUNT_DISC, 0, 0x00];
...
#[test]
fn test_tail_str_invalid_utf8_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
    let authority = Address::new_unique();

    // Invalid UTF-8 in tail str — accepted because UTF-8 re-validation was removed.
    // Owner check proves the owning program wrote the data, and from_utf8_unchecked
    // is sound under the SVM zeroing invariant.
...
#[test]
fn test_tail_str_truncated_multibyte_accepted() {
    let mollusk = setup();
    let account = Address::new_unique();
    let authority = Address::new_unique();
...
#[test]
fn test_adversarial_ix_dynamic_string_prefix_overflow_u8_max() {
    let mollusk = setup();
    let signer = Address::new_unique();

    let mut data = vec![21u8]; // discriminator for dynamic_instruction_check
    data.push(255u8); // u8 prefix claims 255 bytes
...
#[test]
fn test_adversarial_ix_dynamic_string_prefix_overflow() {
    let mollusk = setup();
    let signer = Address::new_unique();

    // u8 prefix = 200, but only 10 bytes of string data follow (and max=8)
    let mut data = vec![21u8];
    data.push(200u8); // prefix claims 200 bytes
...
fn test_adversarial_ix_string_exceeds_max() {
    ...
    let mut data = vec![21u8];
    data.push(9u8); // u8 prefix = 9 (max=8)
